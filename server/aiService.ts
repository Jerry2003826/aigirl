...
- 保持你的独特个性，同时友好协作`;
  ...
  // Add compact language and format instruction (simplified for Function Calling)
  if (language === "en-US") {
    systemPrompt += "\n\nRespond in English with 2-4 short, natural phrases. Be concise (<30 words total), conversational, no robotic terms. If web search provided, use it; otherwise avoid mentioning new movies/TV/news.";
  } else {
    // Default to Chinese (zh-CN) - Function Calling will enforce structure
    systemPrompt += `
...
export async function generateAIResponse(
  ...
  try {
    const response = await provider.generateResponse({
      ...
      ragContext,
      searchEnabled,
      useFunctionCalling: true,  // Enable Function Calling for structured chat responses
    });
