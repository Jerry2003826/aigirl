...
- **Background AI Auto-Reply System (2025-11-05):** Implemented job-based background worker for AI responses when users are offline or in different conversations. Architecture: `ai_reply_jobs` table with status tracking (`pending`/`processing`/`completed`/`failed`), retry mechanism (max 3 attempts), row-level locking (`FOR UPDATE SKIP LOCKED`) to prevent duplicate processing. Worker polls every 3 seconds, processes jobs sequentially, generates AI responses via `generateAIResponse`, and broadcasts to connected clients via WebSocket. Jobs are created automatically when users send messages and survive application restarts. Security: All AI calls use user-provided `customApiKey` from settings.
